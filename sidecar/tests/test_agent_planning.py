...
PLAN = {"tasks": {"t1": {"tool_id": "fs_write", "args": {"path": "x.txt", "content": "a } brace"}, "dependencies": []}}}
...
@pytest.mark.parametrize("tasks", [[], {}, {"a": {"tool_id": "invented", "args": {}}},
...
def test_prose_is_repaired_without_executing_tools(monkeypatch):
    ...
    async def post(self, url, json):
        ...
        content = "The user wants a PDF on the desktop." if len(requests) == 1 else __import__("json").dumps(PLAN)
...
def test_reasoning_without_an_answer_is_not_executed(monkeypatch):
    monkeypatch.setattr(module.engine_manager, "active", SimpleNamespace(base_url="http://model", engine="gguf"))
    monkeypatch.setattr(module, "_active_profile", lambda: None)
    monkeypatch.setattr(module, "_active_tool_specs", lambda: SPECS)
    async def post(self, url, json):
        return httpx.Response(200, request=httpx.Request("POST", url), json={"choices": [{
            "message": {"content": "", "reasoning_content": __import__("json").dumps(PLAN)}}]})
    monkeypatch.setattr(httpx.AsyncClient, "post", post)
...
def test_failed_dependency_is_reported_and_recovery_keeps_distinct_steps(monkeypatch):
    first = module._graph_from_plan({"tasks": {
        "t1": {"tool_id": "fs_write", "args": {"path": "x", "content": "a"}, "dependencies": []},
        "t2": {"tool_id": "fs_write", "args": {"path": "y", "content": "{t1}"}, "dependencies": ["t1"]},
    }}, "goal", SPECS)
    runs = []
    ...
    async def update(graph): pass
    asyncio.run(planner._execute(first, update))
    assert first.tasks["t2"].status == "failed"
    assert len(runs) == 1
    recovered = module._graph_from_plan({"tasks": {
        "t1": {"tool_id": "fs_write", "args": {"path": "z", "content": "ok"}, "dependencies": []},
    }}, "goal", SPECS)
    async def replan(goal): return recovered
